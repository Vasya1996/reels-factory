...
- Существующая команда `script` (старый research-цикл) продолжает работать без изменений поведения — новое добавляется рядом, старое не ломаем (кроме бага `_mentions_theme`).
...
**Files:**
- Modify: `plugins/reels-factory/skills/script/SKILL.md` (добавить выбор пути в начало, секции путей 1 и 2; существующий research-цикл оставить как путь 3 «по рецепту из research»)
...
- [ ] **Step 1: Modify the chat skill**

В `plugins/reels-factory/skills/script/SKILL.md` сразу после заголовка `# Script` вставить:
...
3. **По рецепту из research** — классический цикл (ниже, шаги 1–6), требует
   заполненного `factory/hypotheses.md`.
...
## Путь 3: по рецепту из research (классический цикл)
````

(существующее содержимое скилла становится «Путём 3», его шаги не менять; в
конце «Общих правил» добавить строку: «Пути 1 и 2 не требуют
`factory/hypotheses.md`».)

- [ ] **Step 2: Validate**

Run: `claude plugin validate ./plugins/reels-factory --strict` → OK.
Прочитать итоговый SKILL.md целиком: нет противоречий между шагом 0 и старым
предусловием про hypotheses.md (предусловие перенести внутрь Пути 3).

- [ ] **Step 3: Commit**

```bash
git add plugins/reels-factory/skills/script/SKILL.md
git commit -m "feat(skill): script — явный выбор из трёх путей (свой текст / из сырья / research)"
```
