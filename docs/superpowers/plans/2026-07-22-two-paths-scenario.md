...
Дополнительно в этом же шаге: из шага 9 setup УБРАТЬ платный TTS-смоук
(«привет, это тест» + проверка `_smoke.wav`) — не тратим лимиты ElevenLabs;
оставить только бесплатную проверку `verify --help`. Реальная проверка
голоса произойдёт на первом ролике.
