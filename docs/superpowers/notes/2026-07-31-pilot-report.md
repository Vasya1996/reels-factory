...
покрывающих весь клип (0.5–11.5с) — альфа везде **строго 255** (полностью
непрозрачно), не "почти чёрный фон", а константа. Воспроизвёл отдельно на
...
(topPct=50%) даёт `occPct=80.6%` (пик 93.8% на topPct≈69%) — то есть по
