...
BILLING_DEFAULTS = {
    ...
    "rates": {
        ...
        # Работа Клода в предварительной оценке. Цифры замерены по журналу
        # сессий, а не назначены: подготовка сценария (генерация, хуманизатор,
        # судья) по журналу списаний — $0,07…$0,14 за проход; план монтажа с
        # отбором бироллов на прогоне 462a1c62 (14.08.2026) — $1,24, из них
        # $0,45 планировщик на Sonnet 5 и $0,79 пять сессий судьи на Haiku 4.5.
        # Обе с запасом вверх: оценка на экране цены не должна оказаться ниже
        # того, что спишется по факту.
        "claude_flat_usd_per_reel": 0.15,
        "claude_montage_usd_per_reel": 1.30,
        ...
        # уводит ролик в минус, а человеку названа цифра, которая его не
        # покрывает. Ставка одного прохода рядом остаётся сверяемой с журналом
        # списаний, потому что проходы считаются отдельным множителем.
