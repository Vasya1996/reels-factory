...
BILLING_DEFAULTS = {
    ...
    "rates": {
        ...
        # без наценки 2.0х):
        #   себестоимость одной сессии агента: n=10 проходов, медиана $1.60,
        #     среднее $2.12 (диапазон $0.37–$4.40)
        #   себестоимость ролика (все сессии job): n=7, медиана $2.85,
        #     среднее $3.02 (диапазон $1.26–$5.35)
        #   сессий на ролик: 6 из 7 — одна, 1 из 7 — четыре (job e00b740b,
        #     тот же, что в scenario.py про темп речи — цепочка пересдач из-за
        #     недооценённой длительности, частично устранённой фиксом темпа)
        # claude_montage_usd_per_reel поднят с $1.30 до медианы одной сессии
        # ($1.60): при attempts=2 даёт провизию $3.20 — выше и медианы, и
        # среднего по факту, с запасом, и заметно честнее прежних $2.60
        # (ниже обеих фактических величин).
        "claude_montage_usd_per_reel": 1.60,
