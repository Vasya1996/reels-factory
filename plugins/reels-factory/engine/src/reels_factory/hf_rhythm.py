"""Моменты смены картинки в готовом ролике — измерительный инструмент.

Раньше этот модуль ещё и судил (`rhythm_gates`, гейты D18/D19 по готовому
файлу): решение снято правкой «режиссура» — потолок сцены без смены
переехал в паттерн ритма (`hf_montage.RHYTHMS`) и держит его раскладка ДО
заказа (`hf_phrases.lay_out_scenes`), а не замер ПОСЛЕ рендера, когда
находку уже нечем чинить. `scene_changes` остаётся — тем же счётчиком, каким
меряли ролики приёмки, полезным диагностическим инструментом, просто больше
не гейтом.
...
смен посыпался бы вдвое на каждом ролике, и он перестал бы что-либо отличать.
"""
...
#: 2,31, выше и порога, и пола. Лишняя находка здесь не мешает — список смен
#: не судит гейт с потолком на точный счёт; недолов настоящего среза (тот
#: случай, который чинит этот файл) — портил бы. Побочный эффект
...
#: дважды и ломал калиброванный счёт смен.
SCD_MERGE_GAP = 0.2

#: Планка эталонов: ни одного неподвижного куска длиннее восьми секунд.
#: Раньше рядом стояла и планка смены не реже раза в две секунды
#: (`MAX_SECONDS_PER_CHANGE`, мерилась гейтом `D18_change_rate` по готовому
#: файлу) — снята вместе с `rhythm_gates`: эта планка теперь у паттерна
#: ритма ролика (`hf_montage.RHYTHMS[...]["holdMax"]`, `steady` = ровно то
#: же число), и её держит раскладка ДО заказа
#: (`hf_phrases.lay_out_scenes`), а не замер ПОСЛЕ рендера, когда чинить
#: находку уже нечем.
MAX_STATIC_SPAN = 8.0
...
def _merge_channels(scene_hits: list[float], scd_hits: list[float],
    ...
    `scene_hits` остаются как есть — это откалиброванный счёт первого канала.
    `scd_hits` добавляют находку, только если она дальше `gap` от любой из
    `scene_hits`: иначе один и тот же физический срез, пойманный обоими
    каналами разом, засчитался бы дважды.
    """
