...
# ни разу, и «оплативших» всегда было бы меньше, чем «запустивших сборку».
#
# Порядок обязан повторять порядок экранов: funnel() считает цикл по самому
# дальнему достигнутому шагу и засчитывает ему все предыдущие. Цена теперь
# звучит один раз и ПОСЛЕ сценария — на экране выбора пути, поэтому её шаги
# стоят между утверждением сценария и сборкой, а не перед генерацией, как
# было при отдельном экране цены.
FUNNEL_STEPS = (
    ...
    # Экран выбора пути: цены обоих путей на кнопках и баланс строкой.
    ("price_shown", "Увидели цену"),
    # Нажата кнопка пути, на который денег хватило.
...
ERROR_EVENTS = {
    ...
    # Озвучка — первый платный шаг ролика: не создалась она, и дальше человек
...
class EventStore:
    ...
    def payment_branch(self, since: float = 0.0) -> list[dict]:
        ...
        Оба события обязательны. Одного topup_opened мало: /balance открывают
        и просто так, вне ролика. Одного price_shown — тоже: экран выбора пути
        видят все, включая тех, кому хватило.
        """
