...
#
# `qa_failed` новый код (`bot.py: _process_job`) больше не пишет — с решения
# 05 красный гейт доставляет ролик, а не отказывает. Статус остаётся в списке
# ради СТАРЫХ записей: они ещё лежат в базе, и кнопка продолжения обязана их
# дочитать так же, как раньше.
RESUMABLE_STATUSES = (
    "failed",
    "qa_failed",
    "delivery_failed",
