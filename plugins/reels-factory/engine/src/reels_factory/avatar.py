...
v3-запрос шлёт `background` = то же фото аватара — это НЕ закрепляет сцену
(мягкое кондиционирование, при том же фото — no-op; чистый фон им не
гарантируется, см. комментарий у поля) — и `aspect_ratio: "9:16"` (поле
...
class HeyGenClient:
    ...
    def _body_photo(self, audio_asset_id: str, motion_prompt: str,
        ...
        body = {
            ...
            # Фон НЕ закрепляется этим полем. Avatar IV — диффузионная модель,
            # она генерирует весь кадр; background — мягкое кондиционирование, а
            # при том же фото фактически no-op (проверено 17.07.2026). Поле
            # оставлено как безвредный дефолт. Посторонние люди в кадре этим не
            # лечатся: лечит плита из чистых кадров или webm с альфой.
