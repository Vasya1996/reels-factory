...
# Задача 16: HeyGen кэширует по sha1 звука, а без seed ElevenLabs на одном и
# том же тексте разъезжается до 0.65с — правка титров (тот же текст) заново
# заказывает ведущую, которую можно было переиспользовать. Повторяемость по
# доке ElevenLabs best-effort (не гарантия), на качество голоса не влияет —
# нужна она только ради стабильности звука между прогонами.
DEFAULT_SEED = 42
