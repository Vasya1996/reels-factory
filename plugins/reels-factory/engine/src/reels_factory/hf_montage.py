...
def positions_for(scene: dict) -> tuple[str, ...]:
    ...
    она держит верхнюю треть кадра независимо от того, есть ли в той же
    сцене вставка, и уходить с этой трети не может — а нижние уголки
    (`SCHEMA_SAFE_PRESENTER`) со вставкой уже совместимы: `INSERT_RECTS`
    отдаёт им тот же полноэкранный прямоугольник, что и верхним уголкам, так
    что смотреть на `insert_of` для этой раскладки нечего. Без схемы вставка
    держит кадр сама, а ведущая живёт уголком поверх или половиной над ней
    (`stack`). Без того и другого кадр закрывает сама ведущая.
