...
@functools.lru_cache(maxsize=1)
def schema_safe_presenter() -> tuple[str, ...]:
    """Положения ведущей, при которых схема встаёт во весь свой рост.

    Списка здесь нет — есть счёт. Спрашиваем ту же зону, которой схему ставит
    сборка (`hf_compose.schema_zone`), и оставляем те положения, где зона
    целая: масштаб единица значит, что схеме досталась вся полоса над титром
    и ужимать её не пришлось. Сегодня это `pip-br` и `pip-bl` — их окно лежит
    ниже полосы титра и со схемой не пересекается вовсе; `pip-tr`/`pip-tl`
    оставляют схеме 397 px, `stack` — 136, полнокадровые — полосу под лицом.
    Двинутся прямоугольники (`hf_layout.VIDEO_RECTS`) или полоса титра —
    список двинется сам, а не разойдётся с кадром.

    `none` в список не входит по назначению функции, а не по геометрии: её
    ответ читает `positions_for`, а он выбирает, КУДА ПОСТАВИТЬ ведущую, и
    «нигде» там не выбор.
    """
    from reels_factory.hf_compose import schema_zone
    from reels_factory.hf_layout import PRESENTER_POSITIONS

    return tuple(name for name in PRESENTER_POSITIONS if name != "none"
                 and (schema_zone(name) or {}).get("scale") == 1.0)


def positions_for(scene: dict) -> tuple[str, ...]:
    """Положения ведущей, при которых кадр этой сцены остаётся закрытым.

    Порядок — по убыванию желательности. Схема проверяется раньше вставки:
    она держит полосу над титром независимо от того, есть ли в той же сцене
    вставка, и уходить с этой полосы не может — а положения, при которых
    полоса остаётся целой (`schema_safe_presenter`), со вставкой уже
    совместимы: `INSERT_RECTS` отдаёт нижним уголкам тот же полноэкранный
    прямоугольник, что и верхним, так что смотреть на `insert_of` для этой
    раскладки нечего. Без схемы вставка держит кадр сама, а ведущая живёт
    уголком поверх или половиной над ней (`stack`). Без того и другого кадр
    закрывает сама ведущая.
    ...
    уходит, и обе половины этого правила считает один и тот же
    `FULL_FRAME_PRESENTER`: зона под элемент вида `effect` считается ОТ окна
    ведущей (`hf_layout.effect_rect`), и `full` не оставляет её вовсе, а
    подложка вида `scene` лежит ПОД окном (`.ovl-back`), и полнокадровая
    ведущая закрывает её собой. Прогон
...
def refill_scene(scenes: list[dict], index: int, gaps) -> None:
    ...
    3. Запасная схема из `fallback` закрывает кадр, а ведущая остаётся в
       нижнем уголке: полоса схемы кончается выше полосы титра, а уголок лежит
       ниже неё, и они не спорят (`schema_safe_presenter`). Так у ролика
