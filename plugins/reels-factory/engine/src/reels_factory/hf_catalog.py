...
CONTENT_CHANNELS = ("text_slots", "word_variables", "media_slots")
...
def content_channels(card: dict) -> list[str]:
    ...
    Один вопрос закрывает три места, которые прежде смотрели на карточку
    порознь и не сходились: `hf_montage.filling_element` считал держателем
    кадра любую позицию вида `scene`/`effect` по одному лишь `kind`, не
    спрашивая, есть ли ей чем наполниться, — и `keyframe-scrub-stack` со
    `scroll-feed` (обе без единого слота или переменной) встали держателями
    кадра на живом прогоне `exp-beat-direction` (ролики A и B в
    `work/exp-beat-direction/`) пустыми стопками карточек и пустой лентой
    постов. `hf_gates._element_problems` про канал не спрашивал вовсе, а у 68
    из 151 предлагаемых позиций канала нет (`skeletons-report.md`).

    Три канала, каждый — то место в карточке, куда `hf_compose` кладёт слова,
    число или файл плана, а не готовую демо-строку позиции:
    ...
    Числовая переменная (`type: number`, `role: content`) сюда не входит: её
    канал заводит отдельная работа (PR #86, на момент этого чтения не влит в
    `main`) — когда он появится, канал добавляется в `CONTENT_CHANNELS` и
    сюда же, а не вторым похожим списком.
    """
    channels = []
    if card.get("text_slots"):
...
- `targets` — на ЧТО эту позицию вешают. Такая позиция своей картинки в кадр
  ...
  (`presenter`), вставки сцены (`insert`), слов титра этой сцены (`caption`),
  схемы (`schema`) или собственной разметки позиции (`self`). Мишень пиши в
  поле `target` сцены; если в списке одно значение, код возьмёт его сам. Кадр
  такая позиция НЕ закрывает — сцене всё равно нужны вставка, схема или
  ведущая, а мишени, которой в сцене нет, план не примет (`D36_elements`).
