...
единого сообщения об ошибке), либо разрежет чужой код не по границе. Тем же
файлом чинится и соседняя причина того же обрезания: наши шрифты несут
`unicode-range`-подмножества, догружаемые лениво по первому использованию
(`hf_fonts.py`), а на боте компонента ещё нет ни одного элемента с текстом —
`document.fonts.ready` резолвится, не дождавшись кириллического начертания
Unbounded 800, подгонка кегля меряет запасным шрифтом, и в кадр едет
переполненное слово (`rb0908-university` 32.2с, `АВТОМАТИЗИРОВАТЬ`).
Единственная безопасная точка для правки такого масштаба — файл, который мы
сами пишем и полностью контролируем: `VETTED`. Поэтому `install()` не
доверяет ни их живой версии, ни копии, уже лежащей в staging: `_vet`
сравнивает файл с `VETTED` побайтно и при любом расхождении кладёт `VETTED`
(08.09.2026 копия из старого задания несла все три метки и всё же не несла
эту правку — метки правок больше не критерий, только тождество). Метки
`DATA_HOOK`/`FIT_MARKER`/`CONTRAST_MARKER` остались утверждениями о самом
`VETTED` в тестах.
...
def _vet(target: Path) -> None:
    """Свести проверку и подмену на `VETTED` в одно место — по байтам, не по меткам.

    08.09.2026, прод, задание `rb0908-university`: `.hf-captions/` в папке
    задания несла копию от 31.08 — до PR #105 с прогревом кириллического
    начертания шрифта. Три метки (`DATA_HOOK`/`FIT_MARKER`/`CONTRAST_MARKER`)
    все уже были в этом файле, ранний возврат `install()` принимал его как
    свой, и «ИССЛЕДОВАТЕЛЯМИ» обрезало по краям кадра — тот же класс отказа,
    что чинили метками 07.09.2026 (`rb0909-ai-employee`), только на шаг
    позже: список меток сам по себе не растёт вместе с правками `VETTED`, и
    любой уже застрявший в staging файл проходит его, пока не наберёт ещё
    одну метку, которую ещё не придумали. Критерий не должен расти — он один
    и тот же для любой будущей правки: совпадают байты с `VETTED` — файл наш,
    не совпадают — не наш, независимо от того, какие метки внутри.
    """
    if not VETTED.exists():
        raise RuntimeError(
            f"проверенной копии {COMPONENT} нет в {VETTED} — титр показал бы "
            "их демо-текст, обрезанное слово или буквы ниже порога контраста")
    if target.read_bytes() == VETTED.read_bytes():
        return
    print(f"компонент {COMPONENT} из их реестра отличается от проверенной "
          "копии — беру проверенную копию движка")
...
def install(rdir) -> Path:
    ...
    Если байты `target` не совпадают с `VETTED`, берём проверенную копию:
    молча показать вместо реплик диктора их демо-текст, обрезанное слово или
    буквы ниже их порога контраста — хуже, чем взять файл, который мы сами
    ...
    из прежнего задания), и на только что привезённом `npx add` — одним и тем
    же сравнением по байтам, а не списком меток, который иначе рос бы с
    каждой новой правкой `VETTED` и пропускал бы старую копию, пока в ней не
    заведут метку под ещё не написанную правку (см. `_vet`).
    """
    rdir = Path(rdir)
