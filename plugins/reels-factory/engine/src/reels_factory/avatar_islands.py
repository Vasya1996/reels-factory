...
def _restore_short_faceless(plan: dict, previous: dict,
    ...
    склейка собирает её обратно в одно окно. Осталась причина, которой решение
    агента не касается вовсе, — РАЗРЕЗ окна границей сцены. Окно черновика
    заводится по раскадровке, а границы сцен ставит режиссёр, и на стыке одно
    окно достаётся двум разным решениям: кусок в сцене без ведущей уходит во
    вставку, кусок в соседней остаётся при своём. Любой из двух бывает короче
    `MIN_FULLSCREEN_S` — а такой план `validate_edit_plan` заворачивает уже в
