...
#: показать вовсе.
#:
#: У `metric` недосказанность — не эстетика, а неверный факт. Его блок
#: (`mk-progress-stat.html`) считает число вверх твином GSAP: старт на 0,5 с,
#: длительность 1,6 с, а угасание — на `DUR - 0,5`. Твин интерполирует к
#: `CONFIG.value` непрерывно и достигает его РОВНО в конце своей длительности
#: (счёт кончается на 2,1 с) — это гарантия самого GSAP, не зависящая от
#: величины числа. `Math.round` показывает верную цифру раньше этого мига
#: только для маленьких чисел: у «23» допуск округления (±0,5) — это 2 % от
#: значения, и обгоняющий старт кривой `power2.out` съедает их за первую
#: половину твина. У большого числа («250 000 $», ровно тот пример, что несёт
#: их же карточка, `hf_montage_skill.py:566-572`) тот же допуск — тысячные
#: доли процента, и Math.round показывает не связанную дробь до самого конца
#: твина, то есть до 2,1 с, а не раньше. Старый пол 2,6 с был точной суммой
#: 0,5+1,6+0,5 — тем самым мигом, где твин формально заканчивается и угасание
#: в тот же миг стартует, без единой секунды продержать точное число на
#: экране; `settle_schemas` вдобавок прощает недобор до 0,05 с, то есть могло
#: угаснуть раньше 2,1 с — раньше гарантии GSAP. Для «23 августа» это не
#: стреляло: проверено снятыми кадрами уже отрендеренного `reel.mp4`
#: (rb0907-university, сцена `s-21`, 65,8666–68,4333) — «23 августа» стоит
#: неподвижно и полностью непрозрачно с 67,57 с, за 0,36 с до начала угасания
#: (67,93 с); кадр контактного листа на СЕРЕДИНЕ сцены (67,15 с — их же
#: рецепт «`midpoint` of each scene», `hf_render.py:822-828`) застал число на
#: полпути и не был доказательством дефекта рендера. Риск настоящий для
#: крупных сумм, которые эта же форма обязана уметь показывать. Пол поднят на
#: 0,5 с осадки — тем же приёмом, что у `grid-card-assemble` выше («конец
#: анимации плюс отдельное время постоять»), а не тем же числом:
#: `grid-card-assemble` рискует выглядеть недорисованным, `metric` — соврать
#: зрителю величиной. Инвариант с таймингом самого блока держит
#: `test_метрика_успевает_замереть_до_угасания` (test_hf_schema.py).
_MIN_SECONDS = {
    "metric": lambda n: 3.1,
    "items": lambda n: 3.0,
