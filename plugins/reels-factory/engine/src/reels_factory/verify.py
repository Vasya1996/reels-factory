...
D6 broll_bed  — снят 08.08.2026: мерил звук бирол-слоя консольного конвейера,
                а в пути HyperFrames вставки звука не несут по устройству.
...
def verify_reel(mp4: Path, scenario: dict, dur_fn=None, wh_fn=None, lufs_fn=None,
    ...
    # D6 (звуковая подложка видеоряда) снят 08.08.2026: гейт мерил звук
    # бирол-слоя консольного конвейера, а в пути HyperFrames единственная
    # дорожка — мастер-звук, и вставки звука не несут по устройству
    # (<video muted>). Гейт валился на каждом прогоне, меряя то, чего нет.
    ...
    # SKIP (например D4 без файла субтитров) не считается провалом
