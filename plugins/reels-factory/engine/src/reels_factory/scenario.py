"""Генератор сценария рилса: блогерша раскрывает кейс по теме пользователя.
...
def build_prompt(hypothesis: dict) -> str:
    ...
    parts = [
        ...
        "Ведёт ролик блогерша: живо, по делу, как в разговоре с подписчиком."
        ...
        "т.п.) — блогерша сразу заходит в тему.\n"
        ...
        "фразах — в самом хуке или в первой реплике блогерши в development; тип "
        "хука один из: " + HOOK_TYPES + ";\n"
        "- (опционально) context: короткая завязка сразу после хука;\n"
        "- development: блогерша раскрывает кейс — конкретика, механика, цифры;\n"
        ...
        "У блогерши реплика (speech) обязательна в КАЖДОМ блоке.\n"
