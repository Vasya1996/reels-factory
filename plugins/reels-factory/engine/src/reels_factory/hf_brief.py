...
правил, «Где ведущая».""")
    ...
    catalog_step = ("Пройди сцены ещё раз, теперь по каталогу: посмотри "
                    ...
                    "мишень должна в этой сцене быть. Держателем кадра "
                    "встаёт\n   только позиция с каналом содержимого — слотом "
                    "под слова, число или\n   картинку; без канала это декор "
                    "(её `avoid_when` так и говорит), и одна\n   в сцене, где "
                    "больше нечем закрыть кадр, она план не спасёт "
                    "(`D36_elements`).")
    if avatar_ordered:
...
10. Верни два файла в этой папке, рядом с `BRIEF.md`."""
    ...
    catalog_check = (
        ...
        "(`D36_elements`). Если единственная\n   позиция сцены — декор без "
        "канала содержимого, кадр ею не закрыт: дай\n   сцене вставку, схему "
        "или полную ведущую, либо возьми позицию с каналом\n   "
        "(`D36_elements`).")
    if avatar_ordered:
