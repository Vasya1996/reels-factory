...
титр. Так выглядят передышка и призыв.
...
Поле `insert`: либо `null`, либо объект с полем `query` — **прямой поисковый
запрос стокового видео ПО-АНГЛИЙСКИ**, который уйдёт в сток без изменений.
...
- Финал — **видимый объект призыва**, а не фраза в титре: назови его полем
  `icon` (значок действия — закладка, стрелка, сердце) либо `overlay`
  (соц-карточка).

## Медиа-проход

Расставив сцены, пройди по ним ещё раз — это их обязательный шаг (media
opportunity pass): где в сцене есть сигнал, там назови средство прямо в её
полях.

- Диктор произносит имя, цифру или короткий термин, который стоит закрепить
  глазами → плашка `overlay` с этим текстом.
- Призыв к действию → его видимый объект: `icon` или соц-карточка `overlay`.
- Фоновая сцена → `icon` по смыслу реплики: пустой фон с одним титром —
  сигнал, что средство не названо.
- Кульминация уже несёт вспышку со свушем — её ставит код по биту.
