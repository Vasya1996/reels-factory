...
#: Была `count-up` — 06.09.2026 снята `reels.skip`: канала содержимого у неё
#: нет (`skeletons-report.md`), и образец учил бы позиции, которой в
#: предложенных больше нет.
SAMPLE_ELEMENT = {"name": "decline-chart",
...
""" if any(candidates.values()) else "")
    ...
    retry_block = (
        ...
        "план, который ты положишь сейчас. Файл `frame.md` (оформление, не "
        "поле `frame` у сцены — оно ниже, и заполняется всегда) переписывай, "
        "только если проверка назвала оформление.\n"
...
правил, «Где ведущая».""")
    ...
    catalog_step = ("Пройди сцены ещё раз, теперь по каталогу: посмотри "
                    ...
                    "`word`, буквально как оно звучит в реплике. У позиции\n"
                    "   с числовой переменной без `text_slots` назови само "
                    "число из речи в `variables` —\n   без него код не "
                    "заполнит карточку содержанием. Держателем кадра "
                    "встаёт\n   только позиция с каналом содержимого — "
                    "слотом под слова, числом или\n   картинкой; без канала "
                    "это декор (её `avoid_when` так и говорит), и одна\n   "
                    "в сцене, где больше нечем закрыть кадр, она план не "
                    "спасёт (`D36_elements`).")
    if avatar_ordered:
...
10. Верни два файла в этой папке, рядом с `BRIEF.md`."""
    ...
    catalog_check = (
        ...
        "и это слово в этой сцене звучит. У позиции\n   с числовой "
        "переменной без `text_slots` — само число из речи стоит в\n"
        "   `variables`, внутри её `min`/`max` (`D36_elements`). Если "
        "единственная\n   позиция сцены — декор без канала содержимого, "
        "кадр ею не закрыт: дай\n   сцене вставку, схему или полную "
        "ведущую, либо возьми позицию с каналом\n   (`D36_elements`).")
    if avatar_ordered:
