    ...
    if not avatar_ordered:
        ...
        gaps_block = (
            ...
            + '\n\nСцены на этих фразах стоят с `presenter: "none"` — ведущей '
              "там нет. Кадр держит вставка, и у каждой такой сцены заполни "
              "`fallback`: сток отвечает не всегда, а закрыть кадр там больше "
              "нечем — ведущей в этих секундах не существует.")
    else:
...
`0`–{last_phrase}). Сцена из одной фразы пишется как `[7, 7]`. Диапазоны идут
