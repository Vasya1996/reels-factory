...
Поле `insert`: либо `null`, либо объект с полем `look` — что должно быть на
экране, словами.

Вставка — **живой видео-биролл**, снятый камерой: в кадре что-то происходит.
Описывай look как ДЕЙСТВИЕ с предметом: «рука пишет список в блокноте»,
«пальцы листают документы», «человек печатает на ноутбуке крупным планом».
Неподвижный «символ порядка» или значок не найдётся и не годится.

Вставка иллюстрирует реплику, которую диктор произносит в эти самые фразы:
бери действие из её слов. Говорит «запишите три вопроса» — в кадре рука пишет
...
look и честная реплика.

В шапке ответа заполни `brollContext`: `domain` — про что ролик одной фразой
(по нему судья держит стиль), `anti` — чего в кадрах быть не должно (чужие
темы, с которыми путается поиск).
...
{{"message": "главная мысль ролика одной фразой",
 ...
 "brollContext": {{"domain": "продажи и работа с клиентами в малом бизнесе",
                  "anti": "заводы, роботы, программный код, казино"}},
 "scenes": [
    ...
    {{"id": "s-02", "intent": "…", "beat": "point", "phrases": [1, 2],
      "presenter": "pip-br",
      "insert": {{"look": "рука перелистывает исписанные листы со скриптами",
                 "kind": "video"}},
      "avatarNeeded": true}},
    {{"id": "s-03", "intent": "…", "beat": "turn", "phrases": [3, 3],
      "presenter": "none",
      "insert": {{"look": "человек раскладывает бумаги на переговорном столе",
                 "kind": "video"}},
      "avatarNeeded": false}}
  ]}}
```
