...
def _enforce_face_absence_chain(plan: dict) -> None:
    ...
    делает это по оценке речи `WORDS_PER_SEC` (scenario.py:66) — до правки от
    2026-09-03 та константа стояла на 2.5 и была верхней границей, а не
    серединой: по восьми прод-заданиям слова/с легли в 2.11-2.89, реальная
    озвучка выходила длиннее оценки на -4%…+22% (замер и разбор — в докстринге
    WORDS_PER_SEC). Сейчас константа 2.2, запас больше, но не гарантия: на
    самом медленном из восьми заданий (2.11 слов/с) оценка всё ещё короче
    факта. На +22% цепочка из двух окон по 9.4с оценочных вырастала за лимит.
    Одиночное переросшее окно finalize понижает выше (:2429), а цепочку до
    этой правки не понижал никто — валидатор (:2642) ловил её исключением,
    когда за ролик уже заплачено. Понижаем сами: кадр теряет вставку, но
    ролик доезжает.
    """
    face_absence_start = None
