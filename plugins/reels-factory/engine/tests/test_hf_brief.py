...
def test_правило_наполнения_кадра_живёт_в_одном_месте(tmp_path):
    ...
    # значок описан один раз, с геометрической причиной
    assert text.count("Место значка — верхняя треть") == 1


def test_запасная_схема_объяснена_с_примерами(tmp_path):
    """Сток отвечает не всегда, и сцена без ведущей оставалась голым фоном."""
    text = _text(tmp_path)
    assert "**`fallback` — запасная схема.**" in text
    assert "Заполняй у каждой сцены с бироллом" in text
    assert text.count("<example>") >= 3
    assert '"fallback": {"logo": "instagram", "icon": "chat bubble icon"}' in text


def test_имена_слотов_накладки_берутся_дословно(tmp_path):
    """Три способа уронить сборку накладкой не были названы ни разу."""
    text = _text(tmp_path)
    assert "имя из паспортов ниже, дословно" in text
    assert "имена слотов из паспорта этого блока, дословно" in text
    assert "поля `text` быть не должно вовсе" in text
