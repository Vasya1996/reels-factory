...
def test_правило_позиции_каталога_живёт_в_одном_месте(tmp_path):
    ...
    Образец ответа и порядок работы — не редакция правила: первый показывает
    форму ответа, второй называет проход. B4c показал, что правило без образца
    молчит (агент не заговорил про каталог вовсе), B4d — что правило внутри
    чужого шага не выполняется (агент искал по индексу и ничего не поставил).
    Поэтому `elements` живут в трёх местах и ни в одном не переписывают
    правило: свод правил, образец, шаг «пройди сцены по каталогу».
    """
    for name, kw in (("early", {"avatar_ordered": False,
                                "phrases": GAP_PHRASES}),
                     ("ordered", {"clips": CLIPS, "phrases": GAP_PHRASES})):
        ...
        assert "elements" not in задание.replace(шаги, ""), (
...
def test_положения_без_свободной_зоны_названы_числами_кода(tmp_path):
    ...
    строка = [line for line in раздел.splitlines()
              if "свободный кусок кадра" in line or "при положениях" in line]
    assert строка, "правила про свободную зону в разделе нет"
