...
def test_серия_из_двух_планов_объяснена(tmp_path):
    ...
    assert "не меньше пяти на ролик" in text
...
def test_оплаченная_ведущая_обязана_быть_в_кадре(tmp_path):
    ...
    assert "уже снята и оплачена" in text
    assert "выбрасывает деньги заказчика" in text
...
def test_когда_аватар_остаётся_поверх_биролла(tmp_path):
    ...
    text = _text(tmp_path)
    assert "Сцена со вставкой — ведущая уголком поверх неё" in text
