...
def test_уголок_держится_вставкой_или_схемой(tmp_path):
    ...
    assert "нет ни одного из трёх, ставь `full` или `punch`" in text
    # Третье — полнокадровая позиция каталога: она закрывает кадр наравне со
    # вставкой и схемой (`filling_element`, hf_montage.py), и гейт считает её
    # тем же счётом.
    assert "полнокадровая позиция каталога (`kind: scene`)" in text
    assert "D20_frame_filled" in text
    # Схеме нужна полоса над титром целиком, и целой её оставляют только
    # нижние уголки: их окошко лежит ниже этой полосы.
...
def test_главная_ошибка_не_зовёт_уголок_туда_где_кадр_пуст(tmp_path):
    ...
    # Чем ещё держится уголок — правило одно и живёт у словаря положений
    # (`_positions_block`); здесь только ссылка на него, иначе третий список
    # держателей кадра разошёлся бы с кодом.
    assert "в своде правил, «Где ведущая»" in блок
...
def test_файл_оформления_и_поле_кадра_сцены_не_путаются(tmp_path):
    """Раздел пересдачи говорит «`frame.md` переписывай, только если проверка
    назвала оформление» рядом с полем сцены `frame` («Что вернуть», секция
    ниже) — оба зовутся одним словом, и фразу легко прочесть как разрешение
    не заполнять поле `frame` тоже, хотя оно обязано быть заполнено на любой
    попытке (`D36_elements`). Правило «поле обязательно» уже стоит одним
    местом в «Что вернуть» — здесь нужна только оговорка, какое слово к чему
    относится, без второго изложения того же требования.
    """
    text = _text(tmp_path, phrases=GAP_PHRASES, clips=CLIPS,
                 retry_reason="D28_avatar_bookends: FAIL: s-04 — финал")
    section = (text.split("# Задание на монтаж рилса")[1]
               .split("## Порядок работы")[0])
    assert "Файл `frame.md`" in section, (
        "раздел пересдачи не называет `frame.md` файлом явно")
    assert "не поле `frame`" in section, (
        "раздел пересдачи не разводит файл оформления и поле сцены `frame`")
...
def _гейты(scenes: list[dict], phrases: list[dict], duration: float) -> dict:
    ...
    for scene in scenes:
        ...
        # Решение про кадр `D36_elements` требует у каждой сцены, а планы
        # здешних тестов написаны руками и про другое — про бюджет, пол сцены,
        # промежуток без лица. Недостающее поле дописываем, чтобы гейт судил
        # то, ради чего его позвали. Что образец задания несёт `frame` сам,
        # меряет `test_образец_показывает_решение_про_кадр`.
        scene.setdefault("frame", {"holder": "ведущая", "catalog_checked": [],
...
def test_правило_позиции_каталога_живёт_в_одном_месте(tmp_path):
    ...
    for name, kw in (("early", {"avatar_ordered": False,
        ...
        # Имя проверки `D36_elements` не в счёт: это адрес отказа, а не поле
        # плана — его задание называет и у поля `frame`, и в пункте сверки.
        остальное = (задание.replace(шаги, "").replace(сверка, "")
                     .replace("D36_elements", ""))
        assert "elements" not in остальное, (
...
def test_свод_объясняет_каждый_вид_позиции(tmp_path):
    ...
    # У полнокадровой позиции названо, чем она занимает кадр: она ложится
    # подложкой ПОД окно ведущей (`.ovl-back`, z-index 15 против 20 у окна), и
    # это решает, какой сцене её давать. Прежде свод говорил обратное — «поверх
    # ведущей», — и на трёх живых ранних шагах агент отказывался от `v-world-map`
    # ровно этой причиной.
    assert "под окно ведущей" in правило, (
        "не сказано, что полнокадровая позиция ложится подложкой")
    assert "поверх ведущей" not in правило, (
        "свод по-прежнему обещает, что позиция кроет ведущую")
    # Положения, при которых подложки не видно, — из того же кода, что судит
    # гейт: второго перечня тех же слов в тексте быть не должно.
    for position in FULL_FRAME_PRESENTER:
        assert f"`{position}`" in правило, position
...
def test_сверка_требует_след_прохода_по_каталогу(tmp_path):
    ...
    по ролику. След у прохода теперь свой — поле `frame`, и мерка пункта та же,
    которой судит `D36_elements` до оплаты: сказано или не сказано. Числа
    позиций не меряет ни пункт, ни гейт.
    """
    for name, kw in (("early", {"avatar_ordered": False,
                                "phrases": GAP_PHRASES}),
                     ("ordered", {"clips": CLIPS, "phrases": GAP_PHRASES})):
        ...
        assert "почему взял или не взял" in пункт, (
            f"{name}: пункт не принимает отказ — только названную позицию")
        assert "D36_elements" in пункт, (
            f"{name}: пункт не называет проверку, которой его судят")
        # Числа пункт не называет: гейта «мало позиций» нет и не заводится.
        # Имя проверки не в счёт — это адрес, а не порог.
        assert not re.search(r"\d", пункт.replace("D36_elements", "")), (
            f"{name}: в пункте появилось число — это порог, а не покрытие")
...
РЕПЛИКА_ПРО_ВЕСЬ_МИР = "Клиенты по всему миру: страны мира на одной карте."


def _фразы_с_репликой(индекс: int) -> list[dict]:
    phrases = [dict(item) for item in _фразы(8)]
    phrases[индекс]["text"] = РЕПЛИКА_ПРО_ВЕСЬ_МИР
    return phrases
...
def test_под_фразой_задания_стоят_кандидаты_каталога(tmp_path):
    ...
    Мерка — реплика про весь мир: карта мира честно не обещает область
    (`top5Codes` зашиты в скрипте блока), под «по каждой области» она не
    кандидат.
    """
    from reels_factory.hf_catalog import catalog_cards
    ...
    assert карта, f"карта под репликой про весь мир не названа: {под}"
...
def test_образец_показывает_решение_про_кадр(tmp_path):
    ...
    `_sample_plan`). Поэтому поле стоит у каждой сцены образца и судится тем
    же кодом, что судит план.

    Два исхода прохода по каталогу показаны оба: одна сцена берёт позицию и
    говорит почему, другая называет рассмотренных кандидатов и говорит, почему
    не взяла. Без второй отказ выглядел бы как пропущенный проход — ровно то,
    что было неотличимо во всех шести живых ранних шагах.
    """
    from reels_factory.hf_catalog import catalog_cards
    from reels_factory.hf_gates import frame_choice_problems

    for name, kw in (("early", {"avatar_ordered": False}), ("ordered", {})):
        phrases = _фразы_с_репликой(3)
        задание = _text(tmp_path / name, phrases=phrases, clips=[],
                        duration=16.0, **kw)
        scenes = _board(задание)["scenes"]
        плохо = frame_choice_problems(scenes)
        assert not плохо, f"{name}: образец учит плану, который валит D36: {плохо}"
        рамки = [scene["frame"] for scene in scenes]
        взяли = [рамка for рамка, scene in zip(рамки, scenes)
                 if scene.get("elements")]
        assert взяли and взяли[0]["catalog_checked"], (
            f"{name}: образец не показывает взятую позицию в `frame`")
        отказ = [рамка for рамка in рамки
                 if рамка["catalog_checked"] and рамка not in взяли]
        assert отказ, f"{name}: образец не показывает отказа с причиной"
        # Имена в образце настоящие: выдуманное учило бы плану, который не
        # собрать.
        живые = set(catalog_cards())
        for рамка in рамки:
            assert set(рамка["catalog_checked"]) <= живые, (
                f"{name}: образец называет позицию не из каталога: {рамка}")
