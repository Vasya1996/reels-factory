...
def test_позицию_каталога_ищут_по_смыслу_методом_фреймворка(tmp_path):
    ...
    assert "пиши дословно из индекса" in text
...
def test_три_ответа_на_потерянную_вставку_названы_и_упорядочены(tmp_path):
    """Диагноз 3 и канон: `icon` и позиция каталога описаны как украшения, а не
    как дешёвое спасение кадра, и в плане прогона их ноль. Порядок — по тому,
    сколько средству нужно найти: схему код рисует из текста агента.
    """
    section = _запас(_skill(tmp_path))
    места = [section.find(f"`{поле}`") for поле in
             ("fallback", "elements", "icon")]
