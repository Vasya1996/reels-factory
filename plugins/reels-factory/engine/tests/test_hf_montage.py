...
def test_дата_в_metric_rb0907_university_остаётся_своей_сценой():
    """rb0907-university, 07.09.2026, сцена `s-21` (65,8666–68,4333, реальные
    секунды из джобы на проде): `metric` «23 августа» на 2,5667 с проходит
    пол `metric` (2,6 с) внутри допуска `settle_schemas` (недобор 0,0333 с
    при допуске 0,05 с) и остаётся своей сценой — ровно то, что случилось на
    проде, и на кадре стояло «18 августа» на 67,0 с вместо «23».

    Первое расследование сочло причиной саму длину сцены и подняло пол
    `metric` числом (2,6 → 3,1 с), из-за чего эта же сцена стала бы отдаваться
    соседке — лечение симптома, а не причины. Настоящая причина —
    в `mk-progress-stat`: его твин считал `CONFIG.value` со стартом и
    длительностью, не зависящими от длины сцены, и мог кончиться позже начала
    угасания. `hf_schema.build` теперь узнаёт дату по хвосту при цифрах
    (`_is_dateline`) и не запускает твин вовсе — значение встаёт в кадр сразу,
    отсчитывать нечего. Пол вернулся к 2,6 с, и сцена с датой законно остаётся
    собой, а не уходит соседке."""
    from reels_factory.hf_montage import settle_schemas
    ...
    assert settled == []
    assert [scene["id"] for scene in scenes] == ["s-20", "s-21"]
    assert scenes[1]["endSec"] == pytest.approx(68.4333)
