...
def test_метрика_на_секундах_rb0907_university_отдаёт_сцену_соседке():
    """rb0907-university, 07.09.2026, сцена `s-21` (65,8666–68,4333, реальные
    секунды из джобы на проде): `metric` «23 августа» на 2,5667 с проходила
    старый пол (2,6 с) внутри допуска `settle_schemas` (недобор 0,0333 с при
    допуске 0,05 с). Для этого конкретного числа кадр всё же был верным —
    снятые с уже отрендеренного `reel.mp4` кадры показали неподвижное «23»
    с 67,57 с, за 0,36 с до угасания; на контактном листе «20» стояло только
    в кадре ровно на середине сцены (их же рецепт `midpoint`,
    `hf_render.py:822-828`), а не потому, что счёт не успевал. Тест не про
    этот случай, а про будущий: та же длина сцены под крупную сумму (где
    допуск округления — тысячные доли процента, а не 2 %, как у «23») не
    даёт твину `mk-progress-stat` домотать до точного значения раньше
    угасания. Пол `metric` поднят с запасом (`hf_schema._MIN_SECONDS`), и та
    же длина сцены теперь уходит соседке, а не остаётся с недосчитанным
    числом под угасанием."""
    from reels_factory.hf_montage import settle_schemas

    scenes = [{"id": "s-20", "startSec": 63.0, "endSec": 65.8666,
               "presenter": "full", "phrases": [20, 26], "insert": None},
              {"id": "s-21", "startSec": 65.8666, "endSec": 68.4333,
               "presenter": "pip-bl", "phrases": [27, 27], "insert": None,
               "schema": {"form": "metric", "why": "величина",
                          "value": "23 августа",
                          "label": "дедлайн приёма заявок"}}]

    settled = settle_schemas(scenes)

    assert settled == ["s-21 → соседке"]
    assert [scene["id"] for scene in scenes] == ["s-20"]
    assert scenes[0]["endSec"] == pytest.approx(68.4333)
