...
def test_finalize_разводит_цепочку_соседних_окон_на_точных_таймингах(tmp_path):
    ...
    Черновик разводит цепочку по оценке речи (`WORDS_PER_SEC`, scenario.py:66)
    и укладывается в лимит — 9.0с из 10.0. Реальная озвучка длиннее, те же два
    окна занимают 12.0с, и до этой правки finalize не понижал цепочку, а
    валидатор ронял оплаченную сборку исключением. Блоки здесь заданы вручную
    (start/end ниже), а не через WORDS_PER_SEC — тест воспроизводит числа
    прод-случая, а не саму формулу оценки.
    """
