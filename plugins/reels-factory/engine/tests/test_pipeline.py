...
def test_провал_гейтов_готового_файла_доезжает_до_следующего_прогона(
    ...
    Причина всё ещё обязана доезжать до следующего прогона той же папки — её
    читает как ручная пересборка кнопкой (для настоящей поломки), так и
    прямой повторный вызов `run_make` в этом тесте. С решения 05 `qa_pass:
    False` сам по себе больше не заставляет человека жать «Перезапустить
    сборку»: `bot.py` доставляет такой ролик как обычный (см. `test_bot.py`,
    test_ролик_с_непройденным_qa_всё_равно_отправляется), а retry_reason
    остаётся диагностикой на случай, если кто-то всё же пересоберёт папку
    руками.
    """
