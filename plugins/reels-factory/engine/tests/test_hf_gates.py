...
def test_значение_вне_выбора_ловится_до_заказа(каталог):
    """У `enum` сверяется не только тип, но и сам выбор: варианты каталог
    читает из разметки позиции, а их движок значение вне списка молча заменит
...
#: Титр этой сцены — реплика варианта Б прогона `exp-beat-direction-2`,
#: слово в слово из расшифровки (фразы 2 и 3).
РЕПЛИКА = [{"start": t / 2, "end": t / 2 + 0.4, "text": word}
           for t, word in enumerate(
               ["Ты", "пишешь", "Клоду:", "Составь", "сообщение,", "чтобы",
                "человек", "точно", "согласился", "со", "мной”."])]


def test_слова_позиции_не_пересказывают_титр(каталог):
    """Их полка разводит две работы ровно по этой границе: «Captions add the
    _spoken words_ as a readable subtitle; this adds _designed graphics_ on top
    of the playing video» (`skills/talking-head-recut/SKILL.md:19-20`), а
    карточка там — «not plain captions (the spoken words as text)» (там же:18).

    Прогон `exp-beat-direction-2`, вариант Б, 6,4 с: `chat-message` напечатал
    «Составь сообщение, чтобы человек согласился со мной» ровно тогда, когда
    титр горел теми же словами. Правило до этой работы жило только у форм
    схемы и только прозой задания.
    """
    problems = elements_problems(
        _элементы({"name": "demo-scene",
                   "words": ["Составь сообщение, чтобы человек "
                             "согласился со мной"]}),
        РЕПЛИКА)
    assert len(problems) == 1, problems
    assert "повторяет титр" in problems[0]
    # Слово из реплики агент выбросил («точно»), и сравнение целых строк
    # пересказ бы пропустило: судится самый длинный общий кусок.
    assert "составь сообщение чтобы человек" in problems[0], problems[0]


def test_ярлык_из_одного_слова_титру_не_мешает(каталог):
    """Одно-два слова — это подпись: карточка называет то, о чём речь, и
    совпадение с титром здесь неизбежно и безвредно."""
    assert elements_problems(
        _элементы({"name": "demo-scene", "words": ["Согласился"]}),
        РЕПЛИКА) == []
    assert elements_problems(
        _элементы({"name": "demo-scene", "words": ["Человек точно"]}),
        РЕПЛИКА) == []


def test_пересказ_ловится_и_в_переменной_а_не_только_в_слоте(каталог):
    """Канала подстановки два — слоты разметки и текстовые переменные, — и
    слова варианта Б доехали именно переменной (`chat-message.text`). Судить
    их порознь значило бы поймать одно и пропустить другое."""
    problems = elements_problems(
        _элементы({"name": "count-up",
                   "variables": {"end": 250,
                                 "prefix": "чтобы человек точно"}}),
        РЕПЛИКА)
    assert len(problems) == 1 and "повторяет титр" in problems[0]


def test_без_расшифровки_пересказ_не_судится(каталог):
    """D11 после сборки зовёт тот же код без расшифровки — сравнивать не с
    чем, и правило молчит, как молчит рядом `target_absent`."""
    assert elements_problems(
        _элементы({"name": "demo-scene",
                   "words": ["Составь сообщение, чтобы человек "
                             "согласился со мной"]})) == []
