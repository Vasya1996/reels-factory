...
Сначала выбери язык **текущего ролика** (`language: ru|kk`), затем предложи
два пути ниже. Не переключай язык автоматически по присланному сырью. Явно
чужой язык готового сценария — предложи изменить язык текущего ролика или
прислать другой текст. Для озвучки используй только `voices[language]`; если
такого голоса ещё нет, до `/reels-factory:make` запроси отдельную запись на
выбранном языке.
